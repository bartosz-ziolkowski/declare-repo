	...
	const handleSubmit = async (e) => {
		...
		if (Object.keys(formErrors).length === 0) {
			...
			try {
				...
				if (formData.automata) {
					const automataUrl = await uploadFileToS3(formData.automata);
					formDataToSend.append("automataUrl", automataUrl);
				}
				...
				<div className="px-4 py-5 sm:p-6">
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div>
							...
							>
								...
								<span className="text-gray-400">
									(add if you want metrics calculated, see{" "}
									<Link
						...
						<div>
							...
							/>
							{errors.automata && (
								<p className="mt-2 text-sm text-red-600">{errors.automata}</p>
							)}
						</div>
						<div>
							<label
